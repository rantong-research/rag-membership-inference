...
核心手段是**语义探测（Semantic Probe）**：不直接问「这篇文档在库里吗」，而是把文档内容改写为多个原子事实的 Yes/No 问题，用「回答正确率与 Unknown 率」间接推断成员身份。
...
> Unknown 率 = RAG 回答「Unknown」的比例。非成员文档不在知识库中、检索不到相关内容，故 Unknown 率显著更高（76.67% vs 4.00%）——这是成员推断的核心机制信号，因此用 Unknown 率而非「回答正确率」刻画答案行为（后者把 Unknown 记为错误，会让非成员正确率失真到 17% 这样的低值）。
...
| 指标 | 成员 | 非成员 |
| --- | ---: | ---: |
| 平均 MIA 得分 | 0.5817 | 0.4700 |
| Unknown 率 | 13.61% | 23.49% |
| 平均私有使用率 | 0.7033 | 0.5733 |

| 指标 | 结果 |
| --- | ---: |
| 文档级 ROC-AUC（答案信号） | 0.5776 |
| 文档级 ROC-AUC（私有使用率信号） | 0.6296 |
| Mann-Whitney p（双尾） | 0.1630 |
| Bootstrap 95% CI（得分均值差） | [-0.06, 0.28] |
| 目标文档检索率 | 97.96% |

> 非 DP 基线 AUC 0.9838 → DP 答案信号 AUC 0.5776：成员/非成员差异接近随机，Mann-Whitney 不显著（p=0.16），均值差 Bootstrap 95% CI 含 0。剩余可观测信号主要来自「私有使用率」——即「baseline 不确定 → 触发私有检索」这一决策侧信道，而非被加噪的回答内容本身。

**实现说明（代码已修复，结果需重跑后更新）**

- yes-bias：本地模型几乎只答 yes（回答分布中 `no` 计数为 0）。已在 voter/baseline 提示词加入反 yes-bias 约束（要求显式证据、默认 unknown）。
- 严格逐 token 解释依赖 DashScope 专有的 `partial` 字段；本地 vLLM 不识别该字段且 max_tokens=1 贪心退化为 "TheTheThe..."。本地 `dp_strict_per_token=False` 走「一次性生成 + 逐词 DP 选择」，`partial` 代码保留供 DashScope 使用。

## 8. 结论与下一步

**结论**

1. 非 DP 基线下成员/非成员得分差异显著（AUC 0.9838），语义探测能有效区分成员身份；
2. DP 机制把答案信号 AUC 压到 0.5776（不显著，Bootstrap CI 含 0），显著削弱成员推断；
3. 剩余可观测信号来自「私有使用率」（AUC 0.6296），对应「baseline 不确定 → 触发私有检索」的决策侧信道；
4. 目标文档检索率约 98%，检索已非瓶颈；当前瓶颈是本地模型回答质量（yes-bias、逐 token 解释退化）。

**下一步**

1. 重跑 DP 实验，验证反 yes-bias 提示词与「一次性生成 + 逐词 DP 选择」回退的效果；
2. 对「私有使用率」决策侧信道做 DP 保护（对是否触发私有检索本身加噪或限流）；
3. 扩大样本量（≥100+100）并固定随机种子，量化 AUC 的方差。
